...
select * from store.creditcard;

-- Copy records where matching keys but details have changed

insert into store_stage.creditcard_temp
select a.creditcardid, a.cardtype, a.cardnumber, a.expmonth, a.expyear, from_unixtime(a.modifieddate) modifieddate, current_date()
from store_stage.creditcard a
...
insert into store_stage.creditcard_temp
select a.creditcardid, a.cardtype, a.cardnumber, a.expmonth, a.expyear, from_unixtime(modifieddate) modifieddate, current_date()
from store_stage.creditcard a
where a.creditcardid not in (
    select creditcardid from store.creditcard
    );
